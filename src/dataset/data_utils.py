...
def tokenizer_image_token(prompt, tokenizer, return_tensors=None):
    ...
    if len(prompt_chunks) > 0 and len(prompt_chunks[0]) > 0 and prompt_chunks[0][0] == tokenizer.bos_token_id:
        ...
        prompt_chunks[0] = prompt_chunks[0][1:]

    input_ids.extend(insert_separator(prompt_chunks, [image_token_index]))

    if return_tensors is not None:
        if return_tensors == 'pt':
            # Pad or truncate to a fixed length (e.g., 32)
            if len(input_ids) < 32:
                input_ids = input_ids + [tokenizer.pad_token_id] * (32 - len(input_ids))
...
def preprocess(
    ...
    try:
        ...
        # Handle the error (e.g., skip this sample, use a default value, etc.)
        return None
